...
namespace Finbourne.Insights.Sdk.Extensions.Tests
{
    ...
    public class ApiConfigurationTest
    {
        ...
        [Test]
        public void ApiConfiguration_HasMissingConfig_Missing_InsightsUrl_Returns_True()
        {
            ApiConfiguration config = GetApiConfiguration();
            config.InsightsUrl = String.Empty;
            Assert.IsTrue(config.HasMissingConfig());
        }

        [Test]
        public void ApiConfiguration_MissingConfig_Missing_InsightsUrl_Returns_Correct_String()
        {
            ApiConfiguration config = GetApiConfiguration();
            config.InsightsUrl = String.Empty;
            List<string> missingList = config.GetMissingConfig();
            Assert.That(missingList.Count, Is.EqualTo(1), "GetMissingConfig list contains one item");
            Assert.That(missingList[0], Is.EqualTo(nameof(config.InsightsUrl)), "GetMissingConfig list string correct");
        }
        ...
        private ApiConfiguration GetApiConfiguration() {
            ...
            {
                InsightsUrl = "https://sub-domain.lusid.com/api",
